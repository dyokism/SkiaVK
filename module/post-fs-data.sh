...
cleanup() {
    ...
    if [ "$exit_code" -ne 0 ]; then
        echo "$(date): [FATAL] post-fs-data.sh failed with exit code $exit_code" >> "$LOG_FILE"
        echo "<3>skia_vulkan: post-fs-data.sh failed with exit code $exit_code" >> /dev/kmsg
    fi
}
trap cleanup EXIT

# Wrap execution in a function to avoid 'exit' calls in sourced contexts
run_post_fs_data() {
    ...
    echo "$(date): [INFO] applying debug.hwui.renderer=skiavk via $RESETPROP -n" >> "$LOG_FILE"
